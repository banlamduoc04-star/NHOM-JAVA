...
export default function EditEventPage() {
    ...
    useEffect(()=>{ if(id) getEvent(id).then(e=>setForm({eventName:e.eventName||'',season:e.season||'Summer',eventYear:e.eventYear||'',startDate:e.startDate||'',endDate:e.endDate||'',status:e.status||'Draft',description:e.description||''})).catch(e=>setMessage(e.message)); },[id]);

    async function onSubmit(e: any){e.preventDefault();try{await updateEvent(id,{...form,eventYear:Number(form.eventYear)});router.push('/dashboard/events');}catch(err: any){setMessage(err.message);}}

    if(!id) return <div className="notice error">Thiếu id: /dashboard/events/edit?id=1</div>;
    if(!form && !message) return <Loading/>;
    return <section className="card"><div className="section-title"><h2>Sửa sự kiện #{id}</h2><span>Update</span></div>{message&&<p className="error-text">{message}</p>}{form&&<form className="form-grid" onSubmit={onSubmit}>
