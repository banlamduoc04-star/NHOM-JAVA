...
public class PrizesController {
    ...
    public List<Prize> getPrizes(
        ...
        if (eventId != null) {
            return prizes.findByEventId(eventId);
        }

        if (trackId != null) {
            return prizes.findByTrackId(trackId);
        }
    ...
    public Prize create(
            @Valid @RequestBody CreatePrizeRequest r
    ) {
    ...
    @PostMapping("/award")
    @PreAuthorize("hasAnyRole('EventCoordinator','Admin')")
    public TeamPrize award(
            @Valid @RequestBody AwardPrizeRequest r
    ) {
        if (!teams.existsById(r.teamId())) {
            throw new ResourceNotFoundException(
                    "Không tìm thấy đội thi"
            );
        }
        ...
        if (
                teamPrizes.existsByTeamIdAndPrizeId(
                        r.teamId(),
                        r.prizeId()
                )
        ) {
            throw new IllegalArgumentException(
                    "Giải thưởng đã được trao cho đội này"
            );
        }

        TeamPrize tp = new TeamPrize();

        tp.teamId(r.teamId());
        tp.prizeId(r.prizeId());

        return teamPrizes.save(tp);
    }
}