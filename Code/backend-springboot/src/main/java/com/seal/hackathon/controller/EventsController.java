...
public class EventsController {
    ...
    public EventsController(HackathonEventRepository events, AuditService audit) { this.events = events; this.audit = audit; }

    @GetMapping
    public List<HackathonEvent> getEvents() { return events.findAll(); }
    ...
    public HackathonEvent createEvent(@Valid @RequestBody CreateEventRequest request) {
        ...
        e.eventName = request.eventName(); e.season = request.season(); e.eventYear = request.eventYear();
        e.startDate = request.startDate(); e.endDate = request.endDate(); e.status = request.status() == null ? "Draft" : request.status();
        e.description = request.description(); e.createdBy = SecurityUtil.currentUserId();
